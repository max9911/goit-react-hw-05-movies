...
export const getSearchMovies = async query => {
  try {
    const { data } = await api(
      `/search/movie?api_key=6333a8a701072b16a61a39d74f06cc56&query=${query}&include_adult=false&language=uk-UA&page=1`
    );
    return data;
  } catch (err) {
...
export const getReviewMovInfo = async movieId => {
  const { data } = await api(
    `/movie/${movieId}/reviews?api_key=6333a8a701072b16a61a39d74f06cc56&language=uk-UA&page=1`
  );
  return data;
};
